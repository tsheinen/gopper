...
fn find_known_good_gadgets_on_libc_2_35() -> Result<()> {
    ...
    let gadgets = get_all_gadgets(BYTES)?.into_iter().map(|g| (g.vaddr, g)).collect::<HashMap<_,_>>();
    
    
    assert_eq!(
        gadgets.get(&0x13076C).ok_or(eyre!("couldn't find pop rbx; pop rbp; pop r12; pop r13; pop r14; gadget"))?.decode(BYTES),
